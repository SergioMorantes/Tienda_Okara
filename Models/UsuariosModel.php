<?php 
	...
	class UsuariosModel extends Mysql
	{
		...
		public function selectUsuario(int $idpersona){
			...
			$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email_user,p.direccion,p.barrio,p.ciudad,r.idrol,r.nombrerol,p.status, DATE_FORMAT(p.datecreated, '%d-%m-%Y') as fechaRegistro 
					FROM persona p
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function updateDataFiscal(int $idUsuario, string $strDireccion, string $strBarrio, string $strCiudad){
			$this->intIdUsuario = $idUsuario;
			$this->strDireccion = $strDireccion;
			$this->strBarrio = $strBarrio;
			$this->strCiudad = $strCiudad;
			$sql = "UPDATE persona SET direccion=?, barrio=?, ciudad=? 
						WHERE idpersona = $this->intIdUsuario ";
